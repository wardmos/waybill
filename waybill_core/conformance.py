...
_OBSERVATION_RENDERING_GUIDANCE = (
    ...
    "of these fields. When an outer case or collection contains multiple nested "
    "request or result artifacts, report the outer artifact's explicit decision "
    "goal; do not substitute a nested request's implementation goal. When an "
    "artifact gives a short explicit classification token "
    "for status, copy only that classification token; do not replace or extend it "
    "with a prose current-status sentence or review conclusion. Derive "
    "repo_mismatch only from the current repository identity and state comparison. "
    "An unapplied proposed patch does not create a repository mismatch. An earlier "
    "mismatch that the evidence explicitly marks as reconciled is not a current "
    "mismatch. If the artifact schema is unsupported and repository comparison is "
    "not meaningful, repo_mismatch is true. Bundle safety guidance alone is not an "
