...
from waybill_core.adapter_matrix import CAPABILITY_SCENARIO_REQUIREMENTS
...
""",
    ...
    def _report(
        ...
        *,
        ...
        document = {
            ...
            or ("unsafe_manual" if capability == "export" else "manual"),
            "identity": {
                "adapter": adapter,
                "status": "verified",
    ...
    def test_public_full_matrix_uses_bound_reports_without_model_calls(self) -> None:
        specifications = {
            "claude-code": (
                "claude",
    ...
    def test_report_for_different_executable_fails_binding(self) -> None:
    ...
    def test_partial_report_is_rejected_before_matrix_probe(self) -> None:
    ...
    def test_deterministic_export_report_is_rejected(self) -> None:
        codex, marker = self._fake_executable(
            "codex",
            version_output=f"codex-cli {SYNTHETIC_AGENT_VERSION}",
        )
        ...
        self.assertEqual(2, completed.returncode)
        self.assertIn("export execution_mode must be unsafe_manual", completed.stderr)
        self.assertFalse(marker.exists())

    def test_identity_only_rejects_grok_as_cursor_without_model_call(self) -> None:
    ...
    def test_private_output_includes_resolved_executable_and_report_paths(self) -> None:
        codex, _ = self._fake_executable(
            "codex",
            version_output=f"codex-cli {SYNTHETIC_AGENT_VERSION}",
        )
        ...
        report = json.loads(completed.stdout)
        identity = report["entries"][0]["identity"]
        self.assertEqual(str(codex.resolve()), identity["resolved_path"])
        ...
        self.assertEqual(str(report_path.resolve()), imported["evidence"]["report_path"])

    def test_smoke_dry_run_prints_identity_gate_without_executing_binary(self) -> None:
        agent, marker = self._fake_executable(
            "agent",
